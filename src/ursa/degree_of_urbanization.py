...
from ghsl import load_or_download
...
def find_urban_clusters(pop_array,
                        ...
                        smooth=True, fill=True, min_hole_size=100):
...
def load_input_data_ghs(country, city, path_fua, path_cache, resolution=100):
    ...
    land = load_or_download(bbox_mollweide, 'LAND',
                            data_path=path_cache,
                            resolution=resolution)
    land.rio.set_nodata(0)

    # Get population density grid in km^2
    cell_area = pop.rio.resolution()[0]**2
    pop_density = pop / cell_area * 1e6

    # Get built-up grid
    # Convert to builtup fraction
    built_fraction = built / cell_area

    # Get land fraction grid
    land_fraction = land / cell_area
...
def dou_for_ghs(country, city, path_fua, path_cache, resolution=100):

    (pop_density,
     built_fraction,
     land_fraction) = load_input_data_ghs(country, city,
                                          path_fua, path_cache, resolution)
    ...
    for year in year_list:
        ...
        dou_xr = dou_lvl1(density, builtup,
                          u_center_density, u_center_pop,
                          builtup_trshld,
                          u_cluster_density, u_cluster_pop)

        print("    Getting stats...")
        df_stats = get_stats_df(dou_xr.values,
                                density.values,
                                builtup.values,
                                year)
...
def load_or_process_dou(country, city, path_fua, path_cache, force=False):
    fpath = path_cache / 'dou.tif'
    if fpath.exists() and not force:
        pass
    else:
        dou_for_ghs(country, city, path_fua, path_cache)
