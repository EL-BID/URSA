...
import degree_of_urbanization as dou
...
def load_or_prep_rasters(country, city, path_fua, path_cache):
    fpath = path_cache / 'sleuth_inputs.nc'
    if fpath.exists():
        pass
    else:
        _ = prep_rasters(country, city, path_fua, path_cache)

    return fpath


def prep_rasters(country, city, path_fua, path_cache):
    """SLEUTH inputs are_
    - urban history
    - slope
    ...
    # We are basing SLEUTH in GHS data which comes in Mollweide
    ...
    bbox_latlon = gpd.GeoSeries(
        [bbox_mollweide],
        crs=fua_mollweide.crs
    ).to_crs('EPSG:4326').envelope.buffer(0.05, join_style=2)[0]
    bbox_ee = ru.bbox_to_ee(bbox_latlon)

    # Historic urbanization, obtained from GHSL + DoU processing
    # Extract las 20 years or urbanization for 5 calibration points
    dou_xr = dou.load_or_process_dou(country, city, path_fua, path_cache)
    ...
    # dou_xr.attrs['year_attrs'] = year_attr_dict

    assert dou_xr.min() >= 0
    assert dou_xr.max() >= 1
    assert len(urban_years) >= 4
    # for year, grid in year_attr_dict.items():
    #     assert grid['num_nonurb_pix'] > 0
    ...
    (roads,
     road_i,
     road_j,
     road_dist) = load_roads(bbox_latlon, path_cache, dou_xr)
    ...
    igrid.to_netcdf(path_cache / 'sleuth_inputs.nc')
...
def load_excluded(bbox_ee, bbox_mollweide, path_cache, raster_to_match):
    ...
    protected_xr = load_protected(
        bbox_ee, path_cache, raster_to_match)
...
def load_slope(bbox, path_cache, raster_to_match):
    ...
    slope = rxr.open_rasterio(
        fpath, name='slope', nodata=0.0).squeeze()
    ...
    # Validate
    assert slope.min().item() > 0
    assert slope.max().item() < 99
...
def load_protected(bbox, path_cache, raster_to_match):
    ...
    if not fpath.exists():
        ...
        ).filterBounds(bbox).filter(ee.Filter.eq('STATUS', 'Designated'))
    ...
    protected_xr = protected_xr.rio.reproject_match(
        raster_to_match, Resampling.mode)
...
def load_roads(bbox, path_cache, raster_to_match, d_metric=np.inf):
...
def load_roads_osm(bbox, path_cache, force_download=False):
    ...
    if not edges_path.is_file() or force_download:
        ...
        edges['weight'] = edges.apply(
            lambda x: simplify_road_type(x.highway),
            axis=1)
...
def create_scenario_file(
    ...
    print(f'Created scenario file at: {fpath.absolute()}')
    print(text)
