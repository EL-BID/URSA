...
def plot_map_season(country, city, path_fua,
                    season, year):
    ...
    Map.addLayer(probability_hillshade.clip(bbox_ee),
                 {'min': 0, 'max': 0.8},
                 'Dynamic World',
                 opacity=0.6)
    Map.update_layout(
...
def load_or_get_lc_df(country, city, path_fua, path_cache, force=False):
    fpath = path_cache / 'land_cover.csv'
    if fpath.exists() and not force:
        df = pd.read_csv(fpath, index_col='year')
    else:
        df = get_cover_df(country, city, path_fua, path_cache)

    return df


def plot_lc_year(country, city, path_fua, path_cache, year=2022):
    x_col = 'Área (km²)'
    y_col = 'Tipo de cobertura'

    lc_df = load_or_get_lc_df(country, city, path_fua, path_cache)

    lc_present = (
        lc_df[lc_df.index == year].T.reset_index()
        .rename(columns={'index': y_col, year: x_col})
        .sort_values(x_col, ascending=False).reset_index(drop=True))
...
def plot_lc_time_series(country, city, path_fua, path_cache):
    lc_df = load_or_get_lc_df(country, city, path_fua, path_cache)
    ...
    fig.for_each_trace(
        lambda t: t.update(name=names[t.name])
    )
